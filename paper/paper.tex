...
\textbf{Algorithm 1: Margin Maximization Loss}

\begin{verbatim}
Input: logits L $\in$ R^{B$\times$S$\times$V}, threshold $\tau$
top2 $\leftarrow$ TopK(L, k=2)                          // [B, S, 2]
m $\leftarrow$ top2[:,:,0] - top2[:,:,1]                 // Voronoi margin
gate $\leftarrow$ {(b,s) : m[b,s] < $\tau$}                   // low-margin positions only
L_margin $\leftarrow$ -mean(m[gate])
return L_margin
\end{verbatim}
...
\textbf{Algorithm 2: Fisher Information Distance Loss}

\begin{verbatim}
Input: logits L $\in$ R^{B$\times$S$\times$V}, unembedding W $\in$ R^{V$\times$d}, k
top_k_logits, top_k_idx $\leftarrow$ TopK(L, k)          // [B, S, k]
p_k $\leftarrow$ Softmax(top_k_logits)                    // renormalized top-k probs
W_k $\leftarrow$ Normalize(W[top_k_idx])                  // L2-normalize embeddings [B, S, k, d]
$\Sigma$_k $\leftarrow$ diag(p_k) - p_k p_k^T                   // [B, S, k, k]
for each pair (i, j) where i $\neq$ j:
    $\delta$ $\leftarrow$ W_k[:,:,i,:] - W_k[:,:,j,:]           // embedding difference [B, S, d]
    proj $\leftarrow$ $\delta$ $\cdot$ W_k^T                           // project onto top-k subspace [B, S, k]
    d$^2$_F[i,j] $\leftarrow$ proj^T $\cdot$ $\Sigma$_k $\cdot$ proj           // Fisher distance squared
d_F $\leftarrow$ sqrt(d$^2$_F)
penalty $\leftarrow$ $\Sigma$_{i$\neq$j} p_k[i] $\cdot$ p_k[j] $\cdot$ d_F[i,j] // probability-weighted sum
L_Fisher $\leftarrow$ -mean(penalty)
return L_Fisher
\end{verbatim}
